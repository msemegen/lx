...
struct renderer : private common::non_constructible
{
    ...
    private:
        ...
        bool create(const device::GPU* p_gpu_a, HWND window_handle_a, VkSurfaceKHR vk_surface_a, const Properties& properties_a);
        void release();
    ...
    template<typename Type_x, typename Window_x> static [[nodiscard]] Type_x*
    create(const device::GPU* p_gpu_a, Window_x* p_window_a, const typename Type_x::Properties& properties_a) = delete;
...
template<>
[[nodiscard]] inline renderer::Context* renderer::create<renderer::Context, canvas::Windowed>(const device::GPU* p_gpu_a,
                                                                                              canvas::Windowed* p_window_a,
                                                                                              const Context::Properties& properties_a)
{
    assert(false == context.is_created());

    if (true == context.create(p_gpu_a, *p_window_a, *p_window_a, properties_a))
    {
        return &(context);
    }
...
constexpr renderer::Context::Properties::Queue_family::Kind operator|(renderer::Context::Properties::Queue_family::Kind left_a,
                                                                      renderer::Context::Properties::Queue_family::Kind right_a)
{
    return static_cast<renderer::Context::Properties::Queue_family::Kind>(static_cast<std::uint64_t>(left_a) |
