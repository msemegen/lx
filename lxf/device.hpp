...
struct device : private common::non_constructible
{
    struct GPU : private common::non_copyable
    {
        ...
        GPU(VkPhysicalDevice vk_physical_device,
            bool is_primary,
    ...
    private:
        ...
        Kind from(VkPhysicalDeviceType type) const
        {
            switch (type)
            {
        ...
        Limits from(const VkPhysicalDeviceLimits& limits) const
        {
            return { .max_image_dimension_1d = limits.maxImageDimension1D,
                     .max_image_dimension_2d = limits.maxImageDimension2D,
                     .max_image_dimension_3d = limits.maxImageDimension3D,
                     .max_image_dimensioncube = limits.maxImageDimensionCube,
                     .max_image_arraylayers = limits.maxImageArrayLayers,
                     ...
                     .max_tessellation_generationlevel = limits.maxTessellationGenerationLevel,
                     .max_tessellation_patchsize = limits.maxTessellationPatchSize,
                     ...
                     .framebuffer_color_sample_counts = static_cast<Limits::Sample_count>(limits.framebufferColorSampleCounts),
                     .framebuffer_depth_sample_counts = static_cast<Limits::Sample_count>(limits.framebufferDepthSampleCounts),
                     .framebuffer_stencil_sample_counts = static_cast<Limits::Sample_count>(limits.framebufferStencilSampleCounts),
                     .framebuffer_no_attachments_sample_counts =
                         static_cast<Limits::Sample_count>(limits.framebufferNoAttachmentsSampleCounts),
                     .max_color_attachments = limits.maxColorAttachments,
                     .sampled_image_color_sample_counts = static_cast<Limits::Sample_count>(limits.sampledImageColorSampleCounts),
                     .sampled_image_integer_sample_counts = static_cast<Limits::Sample_count>(limits.sampledImageIntegerSampleCounts),
                     .sampled_image_depth_sample_counts = static_cast<Limits::Sample_count>(limits.sampledImageDepthSampleCounts),
                     .sampled_image_stencil_sample_counts = static_cast<Limits::Sample_count>(limits.sampledImageStencilSampleCounts),
                     .storage_image_sample_counts = static_cast<Limits::Sample_count>(limits.storageImageSampleCounts),
                     .max_sample_mask_words = limits.maxSampleMaskWords,
                     .timestamp_compute_and_graphics = 1u == limits.timestampComputeAndGraphics,
                     ...
                     .line_width_hrange = { limits.lineWidthRange[0], limits.lineWidthRange[1] },
                     .point_size_granularity = limits.pointSizeGranularity,
                     .line_width_granularity = limits.lineWidthGranularity,
                     .strict_lines = 1u == limits.strictLines,
                     .standard_sample_locations = 1u == limits.standardSampleLocations,
    ...
    struct Display : private common::non_copyable
    {
        ...
        Display(HMONITOR monitor_handle,
                bool is_primary,
                std::uint8_t bits_per_pixel,
                std::string_view name,
                common::Rect<std::int32_t, std::uint32_t> logical_rect,
            ...
            , properties { .kind = true == is_primary ? Kind::primary : Kind::additional,
                           .logical_rect = logical_rect,
                           .physical_rect = physical_rect,
                           .bits_per_pixel = bits_per_pixel }
        {
            const std::size_t name_length = name.size() <= CCHDEVICENAME ? name.size() : CCHDEVICENAME;

            std::fill(this->properties.name, this->properties.name + CCHDEVICENAME, '\0');
            std::copy(name.begin(), name.begin() + name_length, this->properties.name);
        }
    ...
    inline static struct filter
    {
        struct GPU
        {
            ...
            struct Limit
            {
                ...
                private:
                    ...
                    template<typename Value_x> friend bool check_limit(const Limit req_limit, Value_x value);
                };
                ...
                static constexpr Value less(std::uint32_t v)
                {
                ...
                static constexpr Value less(std::int32_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value less(std::uint64_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value less(std::int64_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value less(float v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value less_or_equal(std::uint32_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value less_or_equal(std::int32_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value less_or_equal(std::uint64_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value less_or_equal(std::int64_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value less_or_equal(float v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater(std::uint32_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater(std::int32_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater(std::uint64_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater(std::int64_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater(float v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater_or_equal(std::uint32_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater_or_equal(std::int32_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater_or_equal(std::uint64_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater_or_equal(std::int64_t v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
                ...
                static constexpr Value greater_or_equal(float v)
                {
                    static_assert(sizeof(Value::data) >= sizeof(v));
        ...
        struct Display
        {
            ...
            Kind kind;
            common::Extent<std::uint32_t, 2u> logical_size;
            common::Extent<std::uint32_t, 2u> physical_size;
            std::uint8_t bits_per_pixel;
        };

        std::vector<const device::GPU*>
        operator()(std::span<const device::GPU*> devices, const canvas::Windowed* p_window, const filter::GPU& requirements);
        std::vector<const device::Display*> operator()(std::span<const device::Display*> devices, const filter::Display& requirements);
    } filter;
};

constexpr device::GPU::Kind operator|(device::GPU::Kind left, device::GPU::Kind right)
{
    return static_cast<device::GPU::Kind>(static_cast<std::uint64_t>(left) | static_cast<std::uint64_t>(right));
}
constexpr device::GPU::Kind operator&(device::GPU::Kind left, device::GPU::Kind right)
{
    return static_cast<device::GPU::Kind>(static_cast<std::uint64_t>(left) & static_cast<std::uint64_t>(right));
...
constexpr device::GPU::Feature operator|(device::GPU::Feature left, device::GPU::Feature right)
{
    return static_cast<device::GPU::Feature>(static_cast<std::uint64_t>(left) | static_cast<std::uint64_t>(right));
}
constexpr device::GPU::Feature operator&(device::GPU::Feature left, device::GPU::Feature right)
{
    return static_cast<device::GPU::Feature>(static_cast<std::uint64_t>(left) & static_cast<std::uint64_t>(right));
...
constexpr device::GPU::Queue_family::Kind operator|(device::GPU::Queue_family::Kind left, device::GPU::Queue_family::Kind right)
{
    return static_cast<device::GPU::Queue_family::Kind>(static_cast<std::uint64_t>(left) | static_cast<std::uint64_t>(right));
}
constexpr device::GPU::Queue_family::Kind operator&(device::GPU::Queue_family::Kind left, device::GPU::Queue_family::Kind right)
{
    return static_cast<device::GPU::Queue_family::Kind>(static_cast<std::uint64_t>(left) & static_cast<std::uint64_t>(right));
...
constexpr device::GPU::Limits::Sample_count operator|(device::GPU::Limits::Sample_count left, device::GPU::Limits::Sample_count right)
{
    return static_cast<device::GPU::Limits::Sample_count>(static_cast<std::uint64_t>(left) | static_cast<std::uint64_t>(right));
}
constexpr device::GPU::Limits::Sample_count operator&(device::GPU::Limits::Sample_count left, device::GPU::Limits::Sample_count right)
{
    return static_cast<device::GPU::Limits::Sample_count>(static_cast<std::uint64_t>(left) & static_cast<std::uint64_t>(right));
...
constexpr device::Display::Kind operator|(device::Display::Kind left, device::Display::Kind right)
{
    return static_cast<device::Display::Kind>(static_cast<std::uint64_t>(left) | static_cast<std::uint64_t>(right));
}
constexpr device::Display::Kind operator&(device::Display::Kind left, device::Display::Kind right)
{
    return static_cast<device::Display::Kind>(static_cast<std::uint64_t>(left) & static_cast<std::uint64_t>(right));
...
constexpr device::filter::Display::Kind operator|(device::filter::Display::Kind left, device::filter::Display::Kind right)
{
    return static_cast<device::filter::Display::Kind>(static_cast<std::uint64_t>(left) | static_cast<std::uint64_t>(right));
}
constexpr device::filter::Display::Kind operator&(device::filter::Display::Kind left, device::filter::Display::Kind right)
{
    return static_cast<device::filter::Display::Kind>(static_cast<std::uint64_t>(left) & static_cast<std::uint64_t>(right));
...
constexpr device::filter::GPU::Queue_family::Kind operator|(device::filter::GPU::Queue_family::Kind left,
                                                            device::filter::GPU::Queue_family::Kind right)
{
    return static_cast<device::filter::GPU::Queue_family::Kind>(static_cast<std::uint64_t>(left) | static_cast<std::uint64_t>(right));
}
constexpr device::filter::GPU::Queue_family::Kind operator&(device::filter::GPU::Queue_family::Kind left,
                                                            device::filter::GPU::Queue_family::Kind right)
{
